...
import { b2RevoluteJointDef, b2World } from "./box2d";
...
export default function RevoluteJoint({
  ...
  useEffect(() => {
    ...
    const anchA =
      anchorA === "center"
        ? bodyA.GetLocalCenter()
        : fromCanvasToPhysicsVec(anchorA, worldRef.SCALE);
    jointDef.localAnchorA.Set(anchA.x, anchA.y);
    const anchB =
      anchorB === "center"
        ? bodyB.GetLocalCenter()
        : fromCanvasToPhysicsVec(anchorB, worldRef.SCALE);
