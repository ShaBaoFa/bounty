...
class Box2dObject extends Component {
  ...
  createFixturesForInline = (fixDef, body, rects) => {
    ...
    const filteredRects = Array.from(rects).filter(
    ...
    for (let i = 0; i < filteredRects.length; i++) {
      ...
      let fixtureX =
        (curRect.left - leftOffset + (curRect.right - leftOffset)) / 2 -
        centerX;
      let fixtureY = curRect.top - firstRect.top + curRect.height / 2 - centerY;
      fixDef.shape = new PolygonShape();

      fixDef.shape.SetAsBox(
        fromCanvasToPhysics(curRect.width / 2, SCALE),
        fromCanvasToPhysics(curRect.height / 2, SCALE),
        new b2Vec2(
          fromCanvasToPhysics(fixtureX, SCALE),
          fromCanvasToPhysics(fixtureY, SCALE)
        )
      );
  ...
  physicsInit = () => {
    ...
    bodyDef.type = fixed ? BodyType.b2_staticBody : BodyType.b2_dynamicBody;
    ...
    if (shape === "box") {
      ...
      if (rects.length > 1) {
        ...
        fixDef.shape = new PolygonShape();
        fixDef.shape.SetAsBox(
          fromCanvasToPhysics(domWidth / 2, SCALE),
          fromCanvasToPhysics(domHeight / 2, SCALE)
        );
      ...
      fixDef.shape = new CircleShape(fromCanvasToPhysics(domHeight / 2, SCALE));
      body.CreateFixture(fixDef);
      this.setState({ circleRadius: domHeight / 2 });
    } else {
      throw new Error(
        `Unknown shape ${shape}, only box and circle supported for now`
      );
    }
