...
import { b2BodyDef, FixtureDef, b2Body, PolygonShape, BodyType } from "./box2d";
...
const setDomPosition = (dom, width, height, x, y, rotation, SCALE) => {
  ...
  let { left, top } = fromCanvasToDom(
    fromPhysicsToCanvas(x, SCALE),
    fromPhysicsToCanvas(y, SCALE),
    width,
    height
  );
...
const makeEnclosedBox = (width, height, thickness, world, SCALE) => {
  ...
  bodyDef.type = BodyType.b2_staticBody;
  ...
  fixDef.shape.SetAsBox(
    fromCanvasToPhysics(thickness / 2, SCALE),
    fromCanvasToPhysics(height / 2, SCALE)
  );
  ...
  //half width,half height
  fixDef.shape.SetAsBox(
    fromCanvasToPhysics(width / 2, SCALE),
    fromCanvasToPhysics(thickness / 2, SCALE)
  );
